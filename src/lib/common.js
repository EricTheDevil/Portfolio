// file where all the imports are located

// all the imports from the components folder
export { default as Header } from '../components/header/header.jsx';

export { default as Profile } from '../components/profile/profile.jsx';
export { default as ProfileDescription } from '../components/profiledescription.jsx';
export { default as ProfileImage } from '../components/profileimage.jsx';

export { default as Timeline } from '../components/timeline/timeline.jsx';
export { default as Footer } from '../components/footer/footer.jsx';
export { default as FullPageScroller } from '../components/FullPageScroller.jsx';

export { default as Intro } from '../components/intro/intro.jsx';

export { default as Projects } from '../components/projects/projects.jsx';
export { default as Spinner } from '../components/spinner/spinner.jsx';
